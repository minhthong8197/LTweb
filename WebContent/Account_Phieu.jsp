...
Connection connection = DriverManager.getConnection(
        "jdbc:mysql://35.198.251.138:3306/websitehoithao?useUnicode=true&characterEncoding=UTF-8", "root", "1234");

		String ID = request.getParameter("UserID");

    Statement statement = connection.createStatement() ;
    ResultSet resultset = 
    		statement.executeQuery("select UserID, UserName,UserPass,FullName,Quyen,date,PhoneNumber from users where UserID='"+ID+"'") ;  
	...
	<div id="wrapper">
		...
		<div class="row" id="InfoAccount">
			<div class="row" id="Post_Action" style="padding-left: 40px;">
				<h1>
					<strong>THÔNG TIN CÁ NHÂN</strong> <small> |Thông tin tài
						khoản</small>
				</h1>
			</div>
				...
				<form role="form" action="Account_Phieu.jsp">
					...
					<div class="form-group">
						<label>Username:</label> <input type="text" class="form-control"
							...
							value="<%= resultset.getString(2) %>" disabled="disabled">
						<label>Password:</label> <input type="Password" name="Password"
							class="form-control" id="txtPass"
							value="<%= resultset.getString(3) %>" disabled=""> <label>Họ
							...
							id="txtPhone" name="Phone" value="<%= resultset.getString(7) %>">
						<label>Vai trò:</label> <br /> <input type="text"
							class="form-control" id="Quyen" name="Quyen"
							value="<%= resultset.getString(5) %>" disabled="disabled">
						<a href="Sua.jsp?UserID=<%= resultset.getString(1) %>">
